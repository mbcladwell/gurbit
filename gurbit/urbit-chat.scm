(define-module (gurbit urbit-chat)
...
;; (use-module 'urbit-metadata)

;; (defgroup urbit-chat #f
...
;; (define urbit-chat-send-message (ship chat message)
...
;; (define urbit-chat-url-p (string)
;;   (save-match-data
...
;; (define urbit-chat-patp-to-syls (string)
;;   (let ((string (replace-regexp-in-string "[\\^~-]" "" string))
;;         (result '()))
;;     (while (> (length string) 3)
;;       (push (substring string 0 3) result)
;;       (set! string (substring string 3)))
;;     (when (> (length string) 0)
;;       (push string result))
;;     (reverse result)))

;; ;; TODO: real validation
;; (define urbit-chat-patp-p (string)
;;   (and (> (length string) 0)
;;        (= (elt string 0) ?~)))

;; (define urbit-chat-tokenize-message (message)
;;   (let* ((contents ())
;;          (text '())
;;          (not-backticks t)
;;          (push-text
...
;; (define urbit-chat-add-string-properties (string properties)
;;   (add-text-properties 0 (length string)
;;                        properties
;;                        string)
;;   string)

;; (define urbit-chat-color-patp (patp)
...
;; (define urbit-chat-format-url (url)
...
;; (define urbit-chat-format-contents (contents)
...
;; (define urbit-chat-format-node (node)
...
;; (define urbit-chat-format-reference (node)
;;   (let ((post (alist-get 'post node)))
;;     (let-alist post
;;       (concat
...
;; (define urbit-chat-handle-nodes (nodes buffer)
...
;; (define urbit-chat-insert-message (message)
;;   (save-excursion
...
;; (define urbit-chat-self-insert-command (n &optional c)
;;   (interactive "p")
...
;; (define urbit-chat-prompt-start-marker)
;; (define urbit-chat-prompt-end-marker)
;; (define urbit-chat-ship)
;; (define urbit-chat-chat)

;; (define urbit-chat-update-prompt ()
;;   (save-excursion
...
;; (define urbit-chat-start (&optional resource)
;;   (interactive)
;;   (let ((read
...
;;         (progn
;;           (switch-to-buffer buffer #f t)
;;           (urbit-chat-mode ship name))
;;         ))))


;; (aio-define urbit-chat-mode (ship name)
;;   (kill-all-local-variables)
;;   (use-local-map urbit-chat-mode-map)
;;   (set!-local urbit-chat-prompt-start-marker (point-max-marker))
;;   (set!-local urbit-chat-prompt-end-marker (point-max-marker))
;;   (set!-local urbit-chat-ship ship)
;;   (set!-local urbit-chat-chat name)
...
;;   (define ct/send-notification (title msg)
;;     (let ((notifier-path (executable-find "terminal-notifier")))
;;       (start-process
;;        "Message Alert"
;;        "*Message Alert*" ; use `#f` to not capture output; this captures output in background
;;        notifier-path
;;        "-message" msg
;;        "-title" title
;;        "-sender" "dev.hmiller.port"
;;        "-activate" "org.gnu.Emacs")))
;;   (define ct/message-display-native (resource-string body)
;;     (ct/send-notification
;;      (format "Message in %s" resource-string) ; Title
;;      (format "%s" body)))

;; (define urbit-message-update-handler (event)
...
;;                          (set! unread-groups (cl-adjoin resource-name unread-groups))
...
;; (define urbit-chat-resource (resource)
;;   (interactive)
;;   (unless (not resource)
